...
excluded_features = data[['JV_default_PCE', 'HTL_stack_sequence', 'HTL_thickness_list','HTL_deposition_procedure']]

# Remove the excluded features from the clustering data
data_for_clustering = data.drop(['JV_default_PCE', 'HTL_stack_sequence', 'HTL_thickness_list','HTL_deposition_procedure'], axis=1)
...
for i in range(k):
    ...
    filename = f'/personal/HTL-STACK/cluster_{i}_samples.csv'
...
file_path = f'/personal/HTL-STACK/cluster_{cluster_number}_samples.csv'
data = pd.read_csv(file_path)

# Group by multiple sequence columns and calculate mean JV_default_PCE values
grouped_data = data.groupby(['HTL_stack_sequence', 'HTL_thickness_list','HTL_deposition_procedure'])['JV_default_PCE'].mean().reset_index()
...
categories = ['HTL_stack_sequence', 'HTL_thickness_list','HTL_deposition_procedure', 'JV_default_PCE']
N = len(categories)
